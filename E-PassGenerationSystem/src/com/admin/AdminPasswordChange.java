...
public class AdminPasswordChange extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			ResultSet resultset = DatabaseConnection.getResultFromSqlQuery("select password from tbladmin where password='" + cpassword+ "' and uname='" + session.getAttribute("uname") + "'");
			if (resultset.next()) {
				pass = resultset.getString(1);
			}
			if (password.equals(confpass)) {
				if (pass.equals(cpassword)) {
					int i = DatabaseConnection.insertUpdateFromSqlQuery("update tbladmin set password='" + password
